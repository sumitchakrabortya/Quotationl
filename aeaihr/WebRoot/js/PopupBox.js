...
PopupBox.prototype.init=function(){
	...
	boxArray.push("<div id='"+this.divName+"' style='border: 1px solid rgba(0, 0, 0, 0.2);border-radius:5px;margin:0px;padding:0px;'>");	
	boxArray.push("<table width='100%' border='0' cellpadding='0' cellspacing='0'>");
	boxArray.push("  <tr class='handle' ondblclick=\"$('#"+this.name+"MainArea').toggle();\" style='border-bottom: 1px solid rgba(0, 0, 0, 0.1);height:34px;line-height:28px;'>");
	boxArray.push("    <td width='90%' style='padding-top:0px;padding-left:10px;font-size:15px;color:black;'>"+this.title+"</td>");
	boxArray.push("    <td width='31' align='right' style='float: right;text-align: right;'><span style='width:30px;height:30px;vertical-align: middle;margin:0px;text-align:center;display:table-cell;' onMouseMove='PopupBox.onMover(this)' onMouseOut='PopupBox.onMout(this)' onclick='"+this.name+".closeBox()'><img id='"+this.closeBtnId+"' src='"+PopupBox.contextPath+"images/close.gif' width='15' height='15' style='margin:0px;cursor:pointer' alt='关闭窗口' title='关闭窗口'/></span></td>");
	boxArray.push("  </tr>");
...
PopupBox.prototype.sendRequest=function(url,args){
	...
	var draggie = $("#"+this.divName).draggabilly({
		handle: '.handle'
	});		
}
