...
function showMessage(msg){
	$('#errorMsg').html('<span style="float:left;color:#FF0000;">'+msg+'</span><span style="float:right;color:#000">双击关闭提示</span>');
	$('#errorMsg').show();
}
...
function showSplash()
{
	if (!splashOnShowing){
		...
		if (showMessage){
			$('#infoTD').html(info);
			$('#messageDiv').css({"z-index":9999,display:'block'});	
		}	
		$('#splashLayer').css({"z-index":900,display:'block',opacity:'0.5'});
		splashOnShowing = true;
	}
}
function hideSplash()
{
	if ($('#splashLayer')){
		$('#splashLayer').hide();
		$('#messageDiv').hide();
		splashOnShowing = false;	
	}
}
function initMaskLayer(){
	$("<div id='splashLayer'></div>").appendTo("body");

	$('#splashLayer').css({display:'none',position:'absolute','background-color':'#EEE',height:'100%',width:'100%',top:'0px',left:'0px'});
	$("<div id='messageDiv' style='position: absolute;display:none;left:50%;top:50%;margin-left:-125px;margin-top:-120px;z-index:999;'><table width='250' border='0' align='center' cellpadding='0' cellspacing='1' bordercolor='#FF0000' bgcolor='#000000'><tr bgcolor='#FFFFFF'><td height='60' style='font-size:12px;color: black;font-weight: bold;' id='infoTD' align='center' bordercolor='#1860A8' bgcolor='#DEE8F6'>"+waitMsg+"</td></tr></table></div>").appendTo("body");
}
function ele(id){
...
{	
	...
	if ((actionType == copyRequestActionValue || actionType == updateRequestActionValue 
...
function doSubmit(reqOptions) 
{
	...
	if (reqOptions && reqOptions.showMessage  && (reqOptions.showMessage== 'false'
		|| reqOptions.showMessage.toLowerCase() == 'no' || reqOptions.showMessage.toLowerCase() == 'n')){
		showMessage = false;
	}
	...
	if (reqOptions){		
		...
		if (reqOptions.checkUnique && (reqOptions.checkUnique.toLowerCase() == 'true' || 
			reqOptions.checkUnique.toLowerCase() == 'yes' || reqOptions.checkUnique.toLowerCase() == 'y')){
			breakTag = true;			
			showSplash(waitMsg,showMessage);
			$('#'+actionTypeTagId).val('checkUnique');
			postRequest(formTagId,{onComplete:function(responseText){
				if (responseText == ''){
...
function resetBoxPostion(boxId,targetInputId){
	...
	$("#"+boxId).css({top:targetTop,left:targetLeft});
}
...
function wrTitle(caption){
	document.write("<table width=100% style='margin-top:10px;' border=0 align=center cellpadding=0 cellspacing=0 ><tr><td   width=100% align=center background=images/bg5.gif><table border=0 cellpadding=0 cellspacing=0><tr><td>&nbsp;&nbsp;&nbsp;</td><td bgcolor=#FFFFFF style='font-weight:bold;font-size:14px;'>"+caption+"</td></tr></table></td></tr><tr><td height=5></td></tr></table>");
}
...
function enableSave()
{
	...
	$("#"+modifyImgBtnTagId).parent().css({'background-color':''});
...
function enableButton(buttonId){
	...
	if ($(tdObj).data('onmouseout')){
		...
		var HandlerWrapper = function(itemObj,itemHandler){
			...
			this.invoke = function(){
				if (typeof(itemHandler)=='string'){
					this.obj.bind('mouseout',function(){eval(itemHandler);});
				}else{
	...
	if ($(tdObj).data('onmouseover')){
		...
		var HandlerWrapper = function(itemObj,itemHandler){
			...
			this.invoke = function(){
				if (typeof(itemHandler)=='string'){
					this.obj.bind('mouseover',function(){eval(itemHandler)});
				}else{
...
function isSelectedRow(){
	...
	for (var i=0;i < tagIdArray.length;i++)
	{
		...
		if (!isValueValid(tempTagId))return false;
	}
	return true;
}
...
QuoteFilter.prototype.executeFilter = function()
{
	...
	if(window.addEventListener){
		this.inputObj.addEventListener("input",function(){
			...
			inputObjTemp.value = trim(value).replace(/'/g,'').replace(/"/g,'');
		...
		this.inputObj.attachEvent("onblur",function(){
			inputObjTemp.value = trim(inputObjTemp.value).replace(/'/g,'').replace(/"/g,'');			
		});
	...
	for (i=0;i < inputArray.length;i++){
		...
		if (tempInput.type == 'checkbox'){
			if (tempInput.checked){
				$("#"+tempId).val($("#"+tempId).val()+","+tempInput.value);
				$("#"+tempText).val($("#"+tempText).val()+","+tempInput.title);
			}
		}
	}
...
Tab.prototype.init = function(){
	...
	for (var i= 0;i < count;i++)
	{
		...
		if (this.defaultIndex == i){
			innerHtml = "<table height='26' border='0' cellpadding='0' cellspacing='0'><tr><td><img id='"+divId+"_Img01' src='images/tab/tab0_01.gif' width='6' height='26'></td><td style='cursor:pointer;background:url(images/tab/tab0_02.gif) ' id='"+divId+"_Td02' height='26' nowrap>"+caption+"</td><td><img id='"+divId+"_Img03' src='images/tab/tab0_03.gif' width='13' height='26'></td></tr></table>";				
		}
		else{
			innerHtml = "<table height='26' border='0' cellpadding='0' cellspacing='0'><tr><td><img id='"+divId+"_Img01' src='images/tab/tab_01.gif' width='6' height='26'></td><td style='cursor:pointer;background:url(images/tab/tab_02.gif) ' id='"+divId+"_Td02' height='26' nowrap>"+caption+"</td><td><img id='"+divId+"_Img03' src='images/tab/tab_03.gif' width='13' height='26'></td></tr></table>";			
		}
		$(child).html(innerHtml);
		this.tabObj.data("interrupt",false);
		new this.tabHeaderClickHandler(this.tabObj,this.name,this.contentIdPrefix,child);
	}
}
Tab.prototype.interrupt = function(){
	this.tabObj.data("interrupt",true);
}
Tab.prototype.tabHeaderClickHandler = function(headerContainer,tabName,contentIdPrefix,curTab){
	this.curTab = curTab;
	$(this.curTab).bind('click',function(){
		if (headerContainer.data("interrupt") == false){
			...
			{		
				...
				if (document.getElementById(contentId)){
					if (children[i] == curTab){	
						...
						document.getElementById(childId+'_Td02').style.background='url(images/tab/tab0_02.gif) no-repeat';
						...
						document.getElementById(childId+'_Td02').style.background='url(images/tab/tab_02.gif) no-repeat';
...
Tab.prototype.focus = function (param)
{
	var index = 0;
	var focusTabId = null;
...
/***************ThousandFormater start ****************/
function ThousandFormater(inputId){ 
	var obj = $("#"+inputId);
...
function sendRequest(targetUrl,params) {
	...
	if (params){
		...
		if (params.onComplete){
			...
			$.ajax({url:targetUrl,type:'GET',data:theParam,async:asynchronousMode,dataType:curDataType,success:params.onComplete});			
		}else{
			var dataTypeTemp = "script";
			if (params.dataType){
				dataTypeTemp = params.dataType;
			}					
			$.ajax({url:targetUrl,type:'GET',data:theParam,async:asynchronousMode,dataType:dataTypeTemp});		
		}
	}else{
		$.ajax({url:targetUrl,type:'GET',data:theParam,dataType:"script"});
	}
}
function postRequest(form,params)
{
	...
	if (params && params.showSplash){
		isShowSplash = params.showSplash;
	}
	if (isShowSplash){
		if (params && params.messages){
			showSplash(params.messages);
		}else{
			showSplash();
		}
	}	
	...
	if (params){
		if (params.actionType){
			$('#'+actionTypeTagId).val(params.actionType);
		}
		var asynchronousMode = true;
		if (params.asynchronous){
			asynchronousMode = stringToBoolean(params.asynchronous);
		...
		if (params.onComplete){
			...
			$.ajax({url:targetUrl,type:'POST',data:theParam,async:asynchronousMode
				   ,dataType:curDataType,success:params.onComplete});
		}else{
			var dataTypeTemp = "script";
			if (params.dataType){
				dataTypeTemp = params.dataType;
		...
		$.ajax({url:targetUrl,type:'POST',data:theParam,dataType:"script"});
	}
}
/******************************************************
//native ajax end
...
validation.checkIP = function (strIP) {
	if (validation.checkNull(strIP)) return false;
	var re=/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/g 
	if(re.test(strIP))
	{
...
validation.checkMarkAddress = function (strMarkAddress) {
	if (checkNull(strMarkAddress)) return false;
...
EctableMenu.prototype.build = function(){
	...
	$("#__ToolBar__ table tr td").each(function(){
		var text = $(this).text();
		...
		if (!isDisabled && display != 'none'){
			if (!excludeNames.contains(text)){
				...
				if (hotKey && eval("keyMap."+hotKey)){
					...
					eval(menuName+".addItem(text,text,'"+menuName+"',handler);");
				}
...
DTreeMenu.prototype.build = function(){
	...
	var handlerMap = this.handlerMap;
	var keyMap = this.keyMap;
	$("#_TreeToolBar_ tr td").each(function(){
		var handler = $(this).attr('onclick');
		if (!handler){
			handler = $(this).data('onclick');
		}
