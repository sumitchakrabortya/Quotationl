...
var addFactTreeBox;
function showAddFactTreeBoxBox(){
	if (!addFactTreeBox){
		addFactTreeBox = new PopupBox('addFactTreeBox','请选择实际角色',{size:'normal',height:'410px',width:'310px',top:'3px'});
	}
	var handlerId = "SecurityFactTreeSelect";
	var url = 'index?'+handlerId+'&actionType=addFactRequest&resourceType='+$("#resourceType").val()+'&resourceId='+$("#resourceId").val();
	addFactTreeBox.sendRequest(url);
}
function addFactTreeRelation(factIds){
	if (factIds != ""){
		$('#factIds').val(factIds);
		postRequest('form1',{actionType:'addFactAuthRelation',dataType:'script',onComplete:function(responseText){
			hideSplash();
		}});		
	}
}
...
function showAddRoleTreeBoxBox(){
	if (!addRoleTreeBox){
		addRoleTreeBox = new PopupBox('addRoleTreeBox','请选择标准角色',{size:'normal',width:'300px',top:'3px'});
	}
...
function deleteRelation(selectId,param){
	...
	jConfirm('确认要进行删除操作吗？',function(r){
		if (r){
			doSubmit(param);			
		}
	});
...
function emptyRelation(param){
	jConfirm('确认要进行清空操作吗？',function(r){
		if (r){
			doSubmit(param);
		}
	});
...
<table width="100%" border="0">
  <tr>
	...
	<fieldset>
	  <legend style="font-weight: bolder;">标准角色</legend>
...
<%=pageBean.selectValue("roleList") %>
        ...
        <td width="25%" valign="top">
	<fieldset>
	  <legend style="font-weight: bolder;">实际角色</legend>
<table border="0" cellpadding="0" cellspacing="1">
<tr>
   <td onmouseover="onMover(this);" onmouseout="onMout(this);" class="bartdx" align="center" onclick="showAddFactTreeBoxBox();"><input value="&nbsp;" type="button" class="addImgBtn" id="addImgBtn" title="添加" />添加</td>
<td onmouseover="onMover(this);" onmouseout="onMout(this);" class="bartdx" align="center" onclick="deleteRelation('factList',{actionType:'delFactAuthRelation'});"><input value="&nbsp;" type="button" class="delImgBtn" id="delImgBtn" title="删除" />删除</td>   
   <td onmouseover="onMover(this);" onmouseout="onMout(this);" class="bartdx" align="center" onclick="emptyRelation({actionType:'delFactAuthRelations'});"><input value="&nbsp;" type="button" class="cancelImgBtn" id="cancelImgBtn" title="刷新" />清空</td>   
</tr>
</table>
<select name="factList" size="20" id="factList" style="width:100%;height:<%=height%>px;">
<%=pageBean.selectValue("factList") %>
</select>
	</fieldset>          
    </td>
    <td width="25%" valign="top">
...
<input type="hidden" name="factIds" id="factIds" value=""/>
</form>
</body>
</html>
