...
function selectRequest(idValue){
	parent.addFactTreeRelation(idValue);
...
<div id="selectTreeContainer" style="height:330px;overflow-y:auto" ondblclick="doSelectRequest()">
<ul id="tt" class="easyui-tree" data-options="url:'<%=pageBean.getHandlerURL()%>&actionType=retrieveJson&resourceType=<%=pageBean.inputValue("resourceType")%>&resourceId=<%=pageBean.inputValue("resourceId")%>&factId=<%=pageBean.inputValue("factId")%>',method:'get',onlyLeafCheck:true,animate:false,checkbox:true">
</ul>
</div>	
	</td>
  </tr>
